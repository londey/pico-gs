...
**Test:** Two VER documents together provide Test-level coverage for this requirement:
...
- **VER-013** (color-combined output golden image) — Pipeline integration test that renders a scene exercising multi-stage combiner modes through the full RTL hierarchy under Verilator.
...
Acceptance criteria (verified jointly by VER-004 and VER-013):
...
The Q4.12 arithmetic constants ONE (0x1000) and ZERO (0x0000) are shared across the color combiner and alpha blending stages; the RTL centralizes these in `shared/fp_types_pkg.sv`.
