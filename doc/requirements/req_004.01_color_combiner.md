...
- [ ] Configure CC_MODE register via INT-010
- [ ] TEX_COLOR0 and TEX_COLOR1 inputs correctly sourced from texture units
- [ ] SHADE0 and SHADE1 inputs correctly sourced from rasterizer interpolation
- [ ] CONST0 and CONST1 correctly loaded from CONST_COLOR register (0x19)
- [ ] COMBINED in cycle 1 equals cycle 0 output
- [ ] MODULATE mode produces correct `TEX0 * SHADE0` result
- [ ] DECAL mode passes TEX0 through unchanged (C=ONE, B=D=ZERO)
- [ ] LIGHTMAP mode produces `TEX0 * TEX1`
- [ ] MODULATE_ADD mode: two-stage specular composition is correct
- [ ] FOG mode blends combined color toward CONST1 based on fog factor
- [ ] Operations apply per-component (R, G, B, A independently)
- [ ] Verify precision: results match reference within Q4.12 rounding tolerance
- [ ] Single-stage pass-through configuration: cycle 1 with A=COMBINED, B=ZERO, C=ONE, D=ZERO
