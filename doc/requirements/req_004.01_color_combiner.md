...
- **VER-013** (color-combined output golden image) — Pipeline integration test that renders a scene exercising multi-stage combiner modes through the full RTL hierarchy under Verilator.
  The rendered framebuffer is compared pixel-exactly against the approved golden image at `spi_gpu/tests/golden/color_combined.ppm`.
...
The Q4.12 arithmetic constants ONE (0x1000) and ZERO (0x0000) are shared across the color combiner and alpha blending stages; the RTL centralizes these in `fp_types_pkg.sv`.
