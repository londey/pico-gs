...
The framebuffer is backed by the color tile cache (UNIT-013), a 4-way set-associative write-back cache whose lines are 4×4 RGB565 tiles (16 pixels × 16 bits = 256 bits per line).
SDRAM writes for framebuffer pixels occur on dirty cache-line eviction rather than on every tile exit; see UNIT-013 for storage details.

**SDRAM Burst Mode Compatibility:**
The 4×4 block-tiled layout stores pixels in block-contiguous order in SDRAM (INT-011).
The rasterizer walks the bounding box in 4×4 tile order so that all pixels within a tile are processed before advancing to the next, maximizing cache hit rates for in-order rasterization.
The color tile cache (UNIT-013) coalesces writes within each tile and issues a 16-word burst write to SDRAM on eviction, amortizing SDRAM row activation overhead.
