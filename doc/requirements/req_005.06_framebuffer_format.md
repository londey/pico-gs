...
1. Apply ordered dithering if enabled (REQ-005.10): add scaled dither value to fractional bits
...
The framebuffer format remains RGB565 for memory efficiency. Internal pipeline processing uses 10.8 fixed-point (REQ-004.02) with ordered dithering (REQ-005.10) to minimize visible banding during the conversion to RGB565.
