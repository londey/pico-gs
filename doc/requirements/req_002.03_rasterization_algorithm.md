...
**Burst-friendly output**: The edge-walking algorithm produces fragments in scanline order (left-to-right).
With the 4x4 block-tiled framebuffer layout (INT-011), horizontally adjacent fragments within the same tile row map to consecutive addresses within a tile block, enabling burst writes within a tile.
Runs of fragments spanning full tile widths allow SDRAM burst transfers that amortize row activation and CAS latency across multiple words, improving effective fill rate beyond the single-word-access baseline.
See UNIT-007 for arbiter burst mode handling with the tiled address scheme.
