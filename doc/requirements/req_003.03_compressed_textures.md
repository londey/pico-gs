...
The cache intermediate format produced by BC decoders on cache fill is determined by TEXn_FMT.CACHE_MODE (INT-010, INT-032):
CACHE_MODE=0 produces RGBA5652 output (5/6/5/2 bits per channel, with alpha truncated from source precision);
CACHE_MODE=1 produces UQ1.8 per channel output (9 bits per channel) by expanding BC endpoints to 8-bit precision before palette interpolation, preserving the full decoder output precision.
