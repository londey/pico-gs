...
The cache intermediate format produced by BC decoders on cache fill is UQ1.8 per channel (9 bits per channel, 36 bits per texel; see UNIT-011.04).
BC endpoints are expanded to 8-bit precision before palette interpolation, preserving the full decoder output precision.

All BC palette interpolation divisions (e.g., `(2*c0+c1)/3`, `(c0+c1)/2`) are implemented in RTL using shift+add reciprocal-multiply rather than Verilog `/` operators, ensuring deterministic LUT usage across synthesis tools (UNIT-011.04, BC Division note; see DD-039).
