...
The combiner equation is evaluated by UNIT-010 (Color Combiner) via the `cc_mode` register field (INT-010); the dual-texture UV path is provided by the `tri_st0`, `tri_st1`, `tri_q`, `tex0_cfg`, and `tex1_cfg` register outputs wired to UNIT-006 (Pixel Pipeline).
