...
When the texture sampler (UNIT-011) samples textures during rendering, the system SHALL cache decompressed texel data in per-sampler caches to reduce external SDRAM bandwidth and enable single-cycle bilinear texture filtering.
...
- UNIT-011.03 (L1 Decompressed Cache) — per-sampler PDPW16KD 512×36 UQ1.8 banks, XOR set indexing, pseudo-LRU replacement, bilinear bank interleaving (FR-131-1 through FR-131-5)
