...
### REQ-001.04.5: Boot Command Pre-Population
...
### REQ-001.04.6: Boot Screen Rendering
...
### REQ-001.04.7: Boot Screen Presentation
...
### REQ-001.04.8: Boot Sequence Completion Before Host
...
### REQ-001.04.9: Boot Command Persistence Through Reset
...
- REQ-001.04.5: After FPGA power-on, confirm FIFO rd_count equals BOOT_COUNT without any SPI activity.
- REQ-001.04.6: After power-on, capture framebuffer contents and confirm black background with centered RGB Gouraud triangle.
- REQ-001.04.7: After boot sequence completes, read FB_DISPLAY register and confirm it contains the boot screen buffer address.
- REQ-001.04.8: Wait 100 ms after power-on, confirm CMD_EMPTY is asserted (boot commands fully consumed).
- REQ-001.04.9: Power-cycle the FPGA and confirm FIFO contains the expected boot commands immediately after reset, without any SPI or runtime loading.

## Notes

FIFO depth increased from 16 to 32 to accommodate the ~18-command boot sequence while retaining headroom for normal SPI command queueing.
