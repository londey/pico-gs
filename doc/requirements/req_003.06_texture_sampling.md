...
3. On cache hit: read decompressed texels directly from interleaved banks (1 cycle for 2×2 bilinear quad); the texel format is RGBA5652 (CACHE_MODE=0) or UQ1.8 per channel (CACHE_MODE=1) per TEXn_FMT
4. On cache miss: stall pipeline, fetch 4×4 block from SDRAM (CAS latency + burst transfer), decompress to the active cache format, fill cache line, then resume sampling
...
**Note:** Format-specific decoding (FR-024-1 through FR-024-7) occurs only on cache miss during the fill operation, not on every texel access.
Each of the 2 texture samplers has its own independent cache (REQ-003.08).

### FR-024-11: Texture Format Promotion to Q4.12

After cache read, texture pixel data SHALL be promoted to Q4.12 fixed-point for the fragment pipeline.
The promotion path depends on TEXn_FMT.CACHE_MODE (INT-010):
...
**CACHE_MODE=1 (UQ1.8 per channel):**
...
- [ ] Texture cache hit returns correct texels in active cache format (RGBA5652 or UQ1.8)
- [ ] Texture cache miss triggers SDRAM fetch and correct cache fill in active format
...
- [ ] RGBA5652 → Q4.12 promotion produces correct values for all component ranges (CACHE_MODE=0)
- [ ] A2 expansion produces correct Q4.12 alpha values (0, 1365, 2730, 4095) (CACHE_MODE=0)
- [ ] UQ1.8 → Q4.12 promotion produces correct values for all channels (CACHE_MODE=1)
- [ ] UQ1.8 value 0x100 promotes to Q4.12 value 0x1000 (CACHE_MODE=1)
