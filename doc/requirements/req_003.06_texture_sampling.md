...
When the pixel pipeline samples a texel during rasterization, the system SHALL decode the texture data using format-specific decoding for the configured format (BC1, BC2, BC3, BC4, BC5, RGB565, RGBA8888, or R8) and produce a decompressed RGBA8 value for downstream processing.
...
The pixel pipeline SHALL decode BC5 compressed texture data (16 bytes per block) as follows:
...
**Note:** Format-specific decoding (FR-024-1 through FR-024-BC5) occurs only on cache miss during the fill operation, not on every texel access.
Each of the 2 texture samplers has its own independent cache (REQ-003.08).

### FR-024-11: Texture Format Promotion to Q4.12

After cache read, texture pixel data SHALL be promoted from UQ1.8 to Q4.12 fixed-point for the fragment pipeline:
