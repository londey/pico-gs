...
This requirement defines the functional behavior of the texture sampling subsystem (UNIT-011), including format-specific decoding algorithms (UNIT-011.04) and bilinear/trilinear filtering (UNIT-011.02) that convert stored texture data into Q4.12 RGBA values for the rendering pipeline.
...
3. On cache hit: read decompressed texels directly from interleaved banks (1 cycle for 2×2 bilinear quad); the texel format is UQ1.8 per channel (36 bits per texel)
...
- UNIT-011.02 (Bilinear/Trilinear Filter) — bilinear filtering, FR-024-10 cache-aware sampling pipeline
