...
After texture cache read, pixel data SHALL be promoted to Q4.12 using the path matching the active cache format (TEXn_FMT.CACHE_MODE; see INT-010 and INT-032):
...
**CACHE_MODE=1 (UQ1.8 per channel → Q4.12):**
...
- [ ] Texture promotion from RGBA5652→Q4.12 produces correct values for all R5/G6/B5/A2 combinations (CACHE_MODE=0)
- [ ] Texture promotion from UQ1.8→Q4.12 produces correct values for all channel values (CACHE_MODE=1)
- [ ] UQ1.8 value 0x100 promotes exactly to Q4.12 value 0x1000 with no precision loss (CACHE_MODE=1)
