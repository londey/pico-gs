...
The reduction from 4 to 2 texture samplers (REQ-008) allows a 4x increase in per-sampler cache capacity (from 4096 to 16384 texels), significantly improving cache hit rates for large textures and reducing SDRAM bandwidth pressure.
...
- Total EBR budget must remain within ECP5-25K constraints (REQ-051)
...
When texels are stored in the cache, the system SHALL store them in decompressed RGBA5652 format regardless of the source texture format (RGBA4444 or BC1), so that cache reads are format-independent.
...
The EBR budget for the larger caches must be validated against the ECP5-25K total EBR availability (REQ-051).
