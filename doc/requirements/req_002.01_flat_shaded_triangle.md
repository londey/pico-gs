...
When the firmware writes three VERTEX registers with RENDER_MODE configured for flat shading (GOURAUD=0, TEXTURED=0), the system SHALL rasterize a triangle using the single COLOR value from the first vertex for all pixels.
...
- - [ ] Set RENDER_MODE to flat shading (GOURAUD=0, TEXTURED=0)
