...
| DSP slices (18x18) | ≤16 of 28 (57%) | Q4.12 multipliers for color combiner (UNIT-010), Gouraud shade, alpha blend; rasterizer uses incremental addition (no per-pixel DSP multipliers) |
...
DSP budget applies to Q4.12 fixed-point multipliers used in the pixel pipeline (REQ-004.02).
Q4.12 is 16 bits wide (4 integer + 12 fractional); 16×16 multiplication fits within the ECP5 18×18 DSP slice.
The rasterizer (UNIT-005) uses incremental attribute stepping (addition only) and contributes no per-pixel DSP multipliers; DSP resources are concentrated in UNIT-010 (Color Combiner) for the blend equations.
The color combiner requires 4–6 DSP slices for multiply-accumulate (REQ-004.02); the freed rasterizer DSPs provide margin within the ≤16 budget.
