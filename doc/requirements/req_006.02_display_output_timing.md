...
The number of pixels fetched from SDRAM per source scanline is `1 << FB_DISPLAY.FB_WIDTH_LOG2`, which may be less than 640; the display controller scales the source scanline to 640 output pixels using a Bresenham nearest-neighbor scaler (UNIT-008).
