...
When the host issues a MEM_FILL command (REQ-005.08) targeting the active back-buffer base address with a desired fill color, the system SHALL write that fill value to every 16-bit pixel location of the back-buffer using sequential SDRAM burst writes at full available bandwidth.
...
- [ ] MEM_FILL command fills the active back-buffer at full bandwidth
