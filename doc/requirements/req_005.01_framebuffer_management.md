...
Display scanout burst reads reduce arbitration cycles consumed by display prefetch, freeing grant opportunities for the color tile cache on arbiter port 1.
The color tile cache (UNIT-013) coalesces framebuffer writes within each 4×4 tile and issues a 16-word burst write to SDRAM on dirty-line eviction.
When alpha blending is enabled, the cache supplies destination pixels from cached lines; a 16-word burst read from SDRAM is issued only on a cache miss for the required tile.
See UNIT-007, UNIT-008, UNIT-013, and INT-011 for bandwidth budget details.
