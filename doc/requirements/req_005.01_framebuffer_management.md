...
The color tile buffer (UNIT-006) collects pixels within each 4×4 tile and issues a single 16-word burst write per tile on tile exit.
When alpha blending is enabled, the tile buffer also pre-fetches the destination tile with a 16-word burst read on first access to each new tile, replacing the previous per-pixel framebuffer readback.
