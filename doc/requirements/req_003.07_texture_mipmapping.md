...
Mipmapping reduces texture aliasing on surfaces that recede from the viewer and improves GPU texture cache efficiency by matching sample resolution to screen-space footprint.
The ~33% memory overhead per texture is the standard trade-off accepted across GPU hardware generations.
...
- For BC-family textures (BC1, BC2, BC3, BC4, BC5), minimum dimension per level is 4 pixels (BC block minimum)
...
- Size calculation accounts for block-compressed formats (BC1, BC2, BC3, BC4, BC5) and uncompressed formats (RGB565, RGBA8888, R8)

**Acceptance Criteria**:

- Address calculation hardware implemented in UNIT-011.01 (UV Coordinate Processing)
- Supports all formats defined in INT-014: BC1, BC2, BC3, BC4, BC5, RGB565, RGBA8888, R8
...
- UNIT-011.01 (UV Coordinate Processing) — LOD selection (FR-130-2), mipmap address calculation (FR-130-3), mip level clamping
- UNIT-011.02 (Bilinear/Trilinear Filter) — trilinear blending between adjacent mip levels (FR-130-4)

## Notes

Mipmaps improve both visual quality (reduced aliasing) and performance (better cache locality for distant objects).
The ~33% memory overhead is standard in GPU texturing.
