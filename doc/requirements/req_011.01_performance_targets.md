...
Triangle setup latency includes sequential derivative precomputation in UNIT-005.02, which time-multiplexes 28 attribute derivative multiplications through shared MULT18X18D blocks.
...
The Verilator interactive simulator (REQ-010.02) uses a behavioral SDRAM model and may not precisely reflect hardware timing; fill rate and frame time measurements from simulation should be treated as approximations only.
