...
This requirement covers only the FT232H-connected host that drives real FPGA hardware over SPI.
It does not cover the Verilator-based interactive GPU simulator, which drives the GPU model via direct C++ FIFO injection rather than SPI.
The Verilator interactive simulator is a separate capability specified under REQ-010.02.
