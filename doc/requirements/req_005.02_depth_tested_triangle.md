...
- **VER-011** (depth-tested overlapping triangles golden image) — Pipeline integration test that renders two overlapping triangles with known depth values through the full RTL hierarchy.
  The rendered framebuffer is compared pixel-exactly against the approved golden image at `integration/golden/depth_test.png`, confirming that the near triangle occludes the far triangle at every overlapping pixel.
