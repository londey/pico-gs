...
- **VER-011** (depth-tested overlapping triangles golden image) — Pipeline integration test that renders two overlapping triangles with known depth values through the full RTL hierarchy.
  The rendered framebuffer is compared pixel-exactly against the approved golden image at `spi_gpu/tests/golden/depth_test.png`, confirming that the near triangle occludes the far triangle at every overlapping pixel.
- **VER-014** (textured cube golden image) — Supplementary integration test that renders a perspective-projected textured cube using Z_TEST=1 and Z_WRITE=1.
  Back faces of the cube must be occluded by front faces at every covered pixel, providing additional integration coverage of depth-ordered multi-triangle rendering with textured geometry.
  VER-014 complements VER-011 by combining depth testing with texture sampling in the same render pass.

Acceptance criteria (satisfied jointly by VER-002 and VER-011; supplementally by VER-014):
