...
- **Usage:** All RTL unit testbenches (VER-001 through VER-005) and pipeline integration tests (VER-010 through VER-013) run under Verilator.
...
- **Applicable to:** VER-010 (Gouraud triangle), VER-011 (depth-tested overlapping triangles), VER-012 (textured triangle), VER-013 (blended/color-combined output).
...
- **Strategy:** Test vectors for unit tests are checked into `spi_gpu/tests/vectors/` as plain-text CSV files.
  ...
  Large binary assets (textures for VER-012) are generated programmatically by the test harness and are not committed.
