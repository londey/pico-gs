...
2. **Bounding box clamping.**
   Submit triangles that extend partially or fully outside the screen boundaries.
   Verify that the bounding box is clamped to [0, 639] in X and [0, 479] in Y.
   Test cases include: triangle extending past the right edge, past the bottom edge, past the top-left corner, and a triangle fully within bounds (no clamping expected).
...
5. **Degenerate triangle handling.**
   Submit the following degenerate cases and verify correct behavior:
   ...
   - Fully off-screen triangle (all vertices outside [0,639]x[0,479]): rasterizer should emit zero fragments after bounding box clamp results in an empty region.
...
- **Pass Criteria:**
  ...
  - Bounding box coordinates are clamped to [0, 639] x [0, 479] in all test cases.
...
- **Fail Criteria:**
  ...
  - Bounding box exceeds screen bounds or is incorrectly computed.
