...
- `rtl/components/rasterizer/src/raster_hiz_meta.sv`: Hi-Z block metadata store — 8 DP16KD instantiations in 36x512 mode, address decode, slot mux, fast-clear FSM (writes sentinel 9'h1FF to all slots), read port (Port A) for rasterizer Hi-Z queries, write port (Port B) for pixel pipeline min_z updates.
...
- `rtl/components/pixel-write/src/pixel_pipeline.sv`: RTL — drives Hi-Z metadata write port (hiz_wr_en, hiz_wr_tile_index, hiz_wr_new_z_hi) on Z-buffer write cycles.
- `twin/components/pixel-write/src/lib.rs`: Digital twin — Hi-Z metadata update on Z-write (write port consumer).
...
- `rtl/components/rasterizer/src/rasterizer.sv`: RTL — instantiates `raster_hiz_meta`, wires read port to `raster_edge_walk` and exposes Hi-Z write/clear ports to `gpu_top`.
- `rtl/top/gpu_top.sv`: RTL — routes Hi-Z write signals from `pixel_pipeline` to `rasterizer`; detects Z-buffer MEM_FILL to generate `hiz_clear_req`; stalls command FIFO during `hiz_clear_busy`.
