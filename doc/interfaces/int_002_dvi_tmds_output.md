...
- **Pixel Clock:** 25.000 MHz (derived as clk_core / 4 from the unified 100 MHz GPU/SRAM clock; 0.7% deviation from the 25.175 MHz VGA standard, within tolerance for virtually all monitors)
...
All display clocks are generated by the FPGA PLL from a 50 MHz board oscillator:

| Clock | Frequency | Derivation |
|-------|-----------|------------|
| `clk_core` | 100 MHz | PLL 2x from 50 MHz oscillator |
| `clk_pixel` | 25 MHz | clk_core / 4 (synchronous integer divider) |
| `clk_tmds` | 250 MHz | PLL 5x from 50 MHz oscillator (10x pixel clock) |

The pixel clock and core clock have a synchronous 4:1 relationship, which eliminates the need for asynchronous CDC between the display controller and the GPU core/SRAM domain.
