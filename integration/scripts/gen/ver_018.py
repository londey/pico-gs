...
Note: The Skyline texture is fully opaque, so this primarily tests the
color decoding path.  The alpha_gradient_8x8_bc2 texture supplements
this with explicit alpha variation (used by separate unit tests).
"""
...
def generate() -> list[str]:
    ...
    # TEX0_CFG: ENABLE=1, NEAREST, BC2, 256x256, REPEAT, 0 mips
    tex_cfg = pack_tex0_cfg(1, 0, TEX_FMT_BC2, 8, 8, 0, 0, 0, TEX_SLOT_0_512)
    lines.append(emit(ADDR_TEX0_CFG, tex_cfg,
                       "ENABLE=1 NEAREST BC2 256x256 REPEAT base=0x%04X"
                       % TEX_SLOT_0_512))
