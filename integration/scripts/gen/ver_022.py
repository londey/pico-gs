"""VER-022: R8 Texture Golden Image Test — hex generator.

Textured quad with 256x256 R8 grayscale uncompressed Skyline R32 texture atlas
rendered into a 512x480 framebuffer.
White vertex colors; MODULATE produces texture pass-through.
Tests R8 4x4-tiled single-channel grayscale decode path.
"""
...
def generate() -> list[str]:
    lines = []
    lines.append(emit_comment("VER-022: R8 Texture Golden Image Test"))
    lines.append(emit_comment(""))
    lines.append(emit_comment("256x256 R8 grayscale Skyline texture on a 512x480 quad."))
    lines.append(emit_comment("Tests 8bpp 4x4-tiled single-channel decode path."))
    ...
    # Include shared R8 texture data
    lines.append("## INCLUDE: textures/skyline_256x256_r8.hex")
    lines.append(emit_blank())
    ...
    # TEX0_CFG: ENABLE=1, NEAREST, R8, 256x256, REPEAT, 0 mips
    tex_cfg = pack_tex0_cfg(1, 0, TEX_FMT_R8, 8, 8, 0, 0, 0, TEX_SLOT_0_512)
    lines.append(emit(ADDR_TEX0_CFG, tex_cfg,
                       "ENABLE=1 NEAREST R8 256x256 REPEAT base=0x%04X"
                       % TEX_SLOT_0_512))
