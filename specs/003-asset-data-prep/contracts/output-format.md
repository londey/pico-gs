# Output Format Contract: Asset Data Preparation Tool
...
firmware/assets/
├── textures/
│   ├── player.rs              # Rust wrapper
│   ├── player.bin             # Binary RGBA8888 data
│   ├── enemy.rs
│   ├── enemy.bin
│   └── ui_button.rs           # Identifier includes parent dir
│   └── ui_button.bin
└── meshes/
    ├── cube_patch0.rs         # Mesh patch 0
    ├── cube_patch0_pos.bin    # Positions
    ├── cube_patch0_uv.bin     # UV coordinates
    ├── cube_patch0_norm.bin   # Normals
    ├── cube_patch0_idx.bin    # Indices
    ├── sphere_patch0.rs       # Multi-patch mesh
    ├── sphere_patch0_pos.bin
    ├── sphere_patch0_uv.bin
    ├── sphere_patch0_norm.bin
    ├── sphere_patch0_idx.bin
    ├── sphere_patch1.rs
    ├── sphere_patch1_pos.bin
    └── ...
```
...
**Option 1**: Direct module inclusion
```rust
// firmware/src/assets.rs
pub mod textures {
    pub mod player {
        include!("../../assets/player.rs");
    }
}

pub mod meshes {
    pub mod cube_patch0 {
        include!("../../assets/cube_patch0.rs");
    }
}
```

**Option 2**: Flat namespace
```rust
// firmware/src/assets.rs
include!("../../assets/player.rs");
include!("../../assets/cube_patch0.rs");

// Usage: assets::PLAYER_DATA
```

**Option 3**: Conditional compilation
```rust
// firmware/src/assets.rs
#[cfg(feature = "all-assets")]
include!("../../assets/player.rs");

#[cfg(feature = "debug-assets")]
include!("../../assets/debug_cube.rs");
```
...
// 4K-aligned allocation (if needed)
#[repr(align(4096))]
struct AlignedTexture {
    data: [u8; PLAYER_DATA.len()],
}

let aligned = AlignedTexture {
    data: *PLAYER_DATA,
};
```
...
// Verify counts
assert_eq!(positions.len(), CUBE_PATCH0_VERTEX_COUNT * 3);
assert_eq!(uvs.len(), CUBE_PATCH0_VERTEX_COUNT * 2);
assert_eq!(normals.len(), CUBE_PATCH0_VERTEX_COUNT * 3);
assert_eq!(indices.len(), CUBE_PATCH0_INDEX_COUNT);
...
const SPHERE_PATCHES: &[(&[u8], &[u8], &[u8], &[u8], usize)] = &[
    (SPHERE_PATCH0_POSITIONS, SPHERE_PATCH0_UVS, SPHERE_PATCH0_NORMALS, SPHERE_PATCH0_INDICES, SPHERE_PATCH0_VERTEX_COUNT),
    (SPHERE_PATCH1_POSITIONS, SPHERE_PATCH1_UVS, SPHERE_PATCH1_NORMALS, SPHERE_PATCH1_INDICES, SPHERE_PATCH1_VERTEX_COUNT),
    // ... more patches
];
