...
| FR-5.2 | Texture format: RGBA8888 (32 bits per texel, full 32-bit usage) + compressed 8-bit indexed (v2.0) |
| FR-5.2 Note | Textures remain RGBA8888 for quality; framebuffer uses RGB565 for bandwidth efficiency |
...
| FR-6.2 | Color format: RGB565 (16 bits per pixel, stored in lower 16 bits of 32-bit words) |
| FR-6.3 | Size: 1,228,800 bytes per buffer (32-bit words, upper 16 bits unused) |
...
| FR-6.8 | Storage efficiency: RGB565 in [15:0], zeros in [31:16]; future optimization: pack 2 pixels/word |
...
### Framebuffer Format Decision (RGB565)

**RGB565 chosen over RGBA8888**:

**Memory characteristics**:
- Storage: 1,228,800 bytes per buffer (same as RGBA8888, see note below)
- Effective data: 614,400 bytes (RGB565 pixels)
- Waste: 50% (upper 16 bits of each 32-bit word unused)

**Bandwidth**:
- Memory bandwidth: 73.7 MB/s at 60 FPS (32-bit word reads)
- Effective pixel data: 36.9 MB/s (RGB565 data only)
- Compared to RGBA8888: Same memory bandwidth, but only 16-bit data used

**Visual quality**:
- RGB565: 65,536 colors (5+6+5 = 16 bits)
- RGBA8888: 16.8M colors (8+8+8 = 24 bits)
- Trade-off: Minimal perceptual difference for most content
- Alpha channel: Not needed in framebuffer (handled in rasterizer pipeline)

**Current implementation**:
- RGB565 stored in lower 16 bits of 32-bit words
- Upper 16 bits written as zeros
- Simplifies addressing (1 pixel = 1 word)
- Display controller expands RGB565→RGB888 with bit replication

**Future optimization**:
- Pack 2 RGB565 pixels per 32-bit word (true 50% savings)
- Requires more complex addressing in rasterizer and display controller
- Would reduce memory to 614,400 bytes per buffer and bandwidth to 36.9 MB/s

**Why not full RGB565 packing now**:
- Current implementation prioritizes simplicity
- Same addressing as original RGBA8888 design
- Easy migration path (change only bit extraction, not addressing)
- Performance adequate for 640×480 @ 60 FPS
