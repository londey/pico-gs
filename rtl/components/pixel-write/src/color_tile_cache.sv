...
//
// Color-Buffer Tile Cache — 4-way set-associative, write-back, 4×4 tiles
//
// Stores 16-bit RGB565 values in 2 × DP16KD data BRAM.
// Tags stored in 4 × PDPW16KD EBR blocks (one per way) via
// `color_tile_tag_bram.sv`, allowing all four way tags to be read in
// parallel in a single cycle.
// A single-entry "last-tag" FF cache bypasses the tag EBR latency for
// consecutive accesses to the same tile.
//
// Hit latency:
...
// DP16KD, 1×16384 mode).  On cache miss, checks the flag internally to
// decide between SDRAM fill and lazy-fill (zeros).  Bit-clear on color
// write and clear-sweep on `invalidate` are mutually exclusive
...
//
// Mirrors `zbuf_tile_cache.sv` (UNIT-012) with these differences:
//   - 32 sets (vs 128), 2 data DP16KD (vs 8), 9-bit tags (vs 7)
//   - No Hi-Z min-Z feedback path
//   - `invalidate_done` output pulse (not present in UNIT-012)
//   - `flush` / `invalidate` driven directly by register-file pulses

module color_tile_cache (
    input  wire         clk,
    input  wire         rst_n,
    ...
    input  wire         flush,            // Write-back all dirty lines (1 cy pulse)
    output reg          flush_done,       // Flush complete pulse (1 cy)
    input  wire         invalidate,       // Drop all + uninit sweep (1 cy pulse)
    output reg          invalidate_done,  // Invalidate complete pulse (1 cy)

    // ====================================================================
    // SDRAM Interface (arbiter port 1)
    // ====================================================================
    ...
    // Framebuffer Config
    // ====================================================================
    input  wire [15:0]  fb_color_base,    // Color buffer base (upper bits)
    ...
    localparam NUM_SETS       = 32;                // 32 sets (5-bit index)
    localparam NUM_LINES      = NUM_WAYS * NUM_SETS;

    localparam SET_BITS       = 5;                       // $clog2(32)
    localparam TAG_WIDTH      = 14 - SET_BITS;           // 9
    localparam BRAM_ENTRIES   = NUM_WAYS * NUM_SETS * PIXELS_PER_TILE; // 2048
    localparam BRAM_ADDR_W    = 11;                      // $clog2(2048)
    ...
    typedef enum logic [3:0] {
        ...
        S_WR_UPDATE    = 4'd5,  // Write single RGB565 via Port B
        S_WR_FILL_WAIT = 4'd6,
        S_BRAM_RD      = 4'd7,  // Wait 1 cycle for BRAM read after fill
        ...
        S_FLUSH_WB     = 4'd11  // Write back dirty line during flush
    } state_t;
    ...
    // Flag = 1: tile not yet written since last invalidate (lazy-fill).
    ...
    //   (on S_WR_UPDATE).  Mutually exclusive because invalidate flushes
    //   the pipeline before firing.

    (* ram_style = "block" *)
    reg uninit_flags_mem [0:16383];  // Per-tile uninitialized flags
    ...
    // ====================================================================
    // Data BRAM — 2 × DP16KD inferred (dual-port, 16-bit RGB565 values)
    // ====================================================================
    (* ram_style = "block" *)
    ...
    reg                 fill_lazy;        // Lazy-fill flag for current miss
    ...
    // Flush scan complete when at last position.
    //
    // Width-cast the literal 1 to SET_BITS so that the subtraction is
    // performed at SET_BITS width (5 bits).  Without the cast, the
    // unsized integer literal 1 forces the subtraction to 32-bit
    // width, where `NUM_SETS[SET_BITS-1:0] - 1` evaluates to
    // 0xFFFFFFFF (because NUM_SETS[4:0] = 0 for NUM_SETS=32) instead
    // of the intended 5'd31, breaking the comparison.
    wire flush_at_last = (flush_set_ctr ==
                          (NUM_SETS[SET_BITS-1:0] -
                           {{(SET_BITS-1){1'b0}}, 1'b1})) &&
                         (flush_way_ctr == 2'd3);
    ...
    // Suppress unused signal warning: tag_re is unused on the
    // `color_tile_tag_bram` interface (always-on read in the behavioral
    // model; PDPW16KD CER is hard-tied high in the synthesis path).
    wire _unused_tag_re = tag_re;
        ...
        case (state)
            S_IDLE: begin
                if ((rd_req || wr_req) && fast_hit) begin
                    ...
                    // Fast-path hit write: no BRAM activity here
                    // (S_WR_UPDATE handles it)
                end
            ...
            S_EVICT: begin
                ...
                // (2) sdram_wr_data register.  Pre-reads target word_count + 2
                // on each wdata_req pulse.
            ...
            S_WR_UPDATE: begin
                // Port B: write RGB565 value into cache
                portb_we    = 1'b1;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Pulse invalidate_done on the cycle after the uninit sweep
            // completes (uninit_clear_busy falls 1→0).
            invalidate_done <= uninit_clear_busy && !uninit_clear_busy_next
                               && !invalidate;
            ...
            case (state)
                ...
                S_EVICT: begin
                    if (word_count == 4'd0 && !evict_req_sent) begin
                        ...
                        // pre-read and issue burst request.
                        sdram_wr_req   <= 1'b1;
                        sdram_wr_addr  <= tile_byte_addr(
                            evict_tile_idx, 4'd0,
                            fb_color_base, fb_width_log2);
                ...
                S_FILL: begin
                    ...
                    if (sdram_rd_valid) begin
                        ...
                        if (word_count == 4'd15) begin
                            // Fill complete — tag EBR write handled by tag_we_*
                            case (fill_way)
                ...
                // ========================================================
                // S_WR_UPDATE — Write color value + dirty mark
                // ========================================================
                S_WR_UPDATE: begin
