"""VER-020: BC4 Texture Golden Image Test — hex generator.

Textured quad with 256x256 BC4-compressed Skyline R32 texture (grayscale)
rendered into a 512x480 framebuffer.
White vertex colors; MODULATE produces texture pass-through.
Tests BC4 single-channel decode with red replicated to RGB.
"""
...
def generate() -> list[str]:
    lines = []
    lines.append(emit_comment("VER-020: BC4 Texture Golden Image Test"))
    lines.append(emit_comment(""))
    lines.append(emit_comment("256x256 BC4-compressed Skyline (grayscale) on a 512x480 quad."))
    lines.append(emit_comment("Tests single-channel red decode, replicated to RGB."))
    ...
    # Include shared BC4 texture data
    lines.append("## INCLUDE: textures/skyline_256x256_bc4.hex")
    lines.append(emit_blank())
    ...
    # TEX0_CFG: ENABLE=1, NEAREST, BC4, 256x256, REPEAT, 0 mips
    tex_cfg = pack_tex0_cfg(1, 0, TEX_FMT_BC4, 8, 8, 0, 0, 0, TEX_SLOT_0_512)
    lines.append(emit(ADDR_TEX0_CFG, tex_cfg,
                       "ENABLE=1 NEAREST BC4 256x256 REPEAT base=0x%04X"
                       % TEX_SLOT_0_512))
